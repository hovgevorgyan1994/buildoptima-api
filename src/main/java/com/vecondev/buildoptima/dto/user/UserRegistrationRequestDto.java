...
public class UserRegistrationRequestDto {
    ...
    @Schema(description = "User's first name.", example = "John",
            required = true, minLength = 2, maxLength = 20, pattern = "^[A-Za-z]*$")
    ...
    @NotNull
    @Phone
    @Schema(description = "User's phone number.", example = "+37477123456",
            pattern = "^[+]{1}[0-9]{10,14}$")
    private String phone;

    @NotNull
    @Email
    @Schema(description = "User's email: should have valid email format", example = "example@gmail.com")
    private String email;

    @NotNull
    @Password
    @Schema(description = "User's password: should have valid password format, at least one uppercase character, " +
                          "one lowercase character, one digit, one special symbol and no whitespaces!",
            example = "Example1234.", minLength = 8, maxLength = 32)
